...
faceCascade = cv2.CascadeClassifier(
    cv2.haarcascades+'haarcascade_frontalface_default.xml')
...
def face_detect(image, sf, mn):
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.putText(image, 'F-'+str(i), (x, y - 10),
...
def run():
    ...
    if choice == 'Image':
        st.markdown(
            '''<h4 style='text-align: left; color: #d73b5c;'>* Face Detection is done using Haar Cascade XML dataset & OpenCV"</h4>''',
        ...
        if img_file is not None:
            ...
            st.markdown(
                ...
                unsafe_allow_html=True)
            scale_factor = st.slider("Set Scale Factor Value (Lesser the value => Much easily face detection)", min_value=1.1, max_value=1.9, step=0.10, value=1.3)
            min_Neighbors = st.slider("Set Scale Min Neighbors (Lesser the value => Much easily face detection)", min_value=1, max_value=9, step=1, value=5)
